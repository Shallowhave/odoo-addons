...
class ProductTemplate(models.Model):
    ...
    secondary_uom = fields.Boolean(index=True, default=False,string='是否开启第二单位')
    secondary_uom_id = fields.Many2one('uom.uom', '第二单位')
    secondary_uom_ids = fields.Many2many('uom.uom',compute='_compute_uom_id')
    secondary_uom_name = fields.Char(string='第二单位名称', related='secondary_uom_id.name', compute='_compute_uom_name',readonly=True)
    secondary_product_qty = fields.Float(string='第二单位在手',compute='_compute_secondary_quantities', search='_search_secondary_qty_available')
    ...
    def _compute_is_safty(self):
        for product in self:
            ...
            if product.safty_rule == 'not_note1':
    ...
    def _compute_lot_weight(self):
        for product in self:
            quants = self.env['stock.quant'].sudo().search([('product_id.product_tmpl_id','=',product.id),('location_id.usage','=','internal')])
            product.lot_weight = sum(quants.mapped('lot_weight'))
